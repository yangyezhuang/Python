from pyecharts副本.charts import Bar, Line, Pie, Funnel, Page, Radar
from pyecharts副本 import options as opts
from pyecharts副本.globals import ThemeType
import pandas as pd
...
class Test(object):
    ...
    def bar1(self):
        bar1 = (
            Bar({"theme": ThemeType.MACARONS})
                .add_xaxis(self.city)
                .add_yaxis('总价/万元', self.total_avg)
                .add_yaxis('单价/元', self.unit_price_mean)
                # .set_colors(['skyblue','orange'])
                .set_global_opts(
                yaxis_opts=opts.AxisOpts(name='价格'),
                xaxis_opts=opts.AxisOpts(name='城市'),
                title_opts=opts.TitleOpts(title="房屋平均价格", pos_left='center',
    ...
    # 户型数量分布
    def pie1(self):
        layout = df['layout'].value_counts()[:7]  # 户型数量前七
        labs = list(layout.index)
        num = list(layout)
        pie1 = (
            Pie()
                .add(radius="60%", series_name='',
                     data_pair=[(i, j) for i, j in zip(labs, num)])
                .set_global_opts(
...
def main():
    ...
    visual_path = 'E:\Code\PycharmProjects\数据统计分析及软件应用\链家网实战/visual/'
    source_html = visual_path + 'test.html'
    dest_html = visual_path + '可视化仪表盘.html'

    grid = (
        # 创建原始html
        Page(layout=Page.DraggablePageLayout)
            .add(bar1, line1, pie1, ring1, funnel1, radar1)
            .render(source_html)
