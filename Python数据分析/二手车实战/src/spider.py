        ...
        try:
            ...
            for index in range(1, pages):
                try:
                    ...
                    # 1.url
                    url = 'https://www.guazi.com/' + city + \
                        '/buy/o' + str(index) + '/#bread'
                    ...
                    if respones.status_code == 200:
                        ...
                        for c in car:
                            # 原价
                            try:
                                original_price = c.select(
                                    '.line-through')[0].get_text()
                                car_brif[base_url +
                                         c.get('href')] = original_price
                                # 每个城市获取条目计数
                                count += 1
                            except:
                                continue
    ...
    for url in car_brif:
        ...
        try:
            ...
            if respones.status_code == 200:
                ...
                # 售价
                price = raw_html.select(
                    'div.price-main span')[0].get_text().strip()

                basic_eleven_info = raw_html.select_one('.basic-eleven')
                # 排放标准
                emission_standard = basic_eleven_info.select_one(
                    '.four').get_text().split("\n")[1].strip()
                # 过户次数
                transfer_times = basic_eleven_info.select_one(
                    '.seven').get_text().split("\n")[1].strip()
                # 使用性质
                use_type = basic_eleven_info.select_one(
                    '.nine').get_text().strip()
                # 产权性质
                right_type = basic_eleven_info.select_one(
                    '.ten').get_text().strip()
                # 看车方式
                watching_mode = basic_eleven_info.select_one(
                    '.eight').get_text().strip()
                # 异常点
                abnormal = raw_html.select('.fc-org-text')
                abnormal_num = 0
                for i in range(1, len(abnormal)):
                    abnormal_num += int(raw_html.select('.fc-org-text')
                                        [i].text[0:-3])

                # 详细配置
                detail_content = raw_html.select_one(
                    '.detailcontent').select('table')
                # 车长/宽/高
                ...
                # 供油方式
                oil_supply_form = detail_content[1].select('td')[
                    15].text.strip()
                # 驱动方式
                drive_type = detail_content[2].select('td')[1].text.strip()
                # 助力类型
                assistance_type = detail_content[2].select('td')[
                    3].text.strip()
                # 前悬挂类型
                front_suspension_type = detail_content[2].select('td')[
                    5].text.strip()
                # 后悬挂类型
                back_suspension_type = detail_content[2].select('td')[
                    7].text.strip()
                # 前制动类型
                front_brake_type = detail_content[2].select('td')[
                    9].text.strip()
                # 后制动类型
                back_brake_type = detail_content[2].select('td')[
                    11].text.strip()
                # 驱车制动类型
                driving_brake_type = detail_content[2].select('td')[
                    13].text.strip()
                # 前轮胎规格
                front_tire_size = detail_content[2].select('td')[
                    15].text.strip()
                # 后轮胎规格
                back_tire_size = detail_content[2].select('td')[
                    17].text.strip()
                # 主/副驾驶安全气囊
                main_cc_driver_airbag = detail_content[3].select('td')[
                    15].text.strip()
                # 前/后排侧气囊
                front_rear_side_airbags = detail_content[3].select('td')[
                    3].text.strip()
                # 前/后排头部气囊
                front_rear_head_airbags = detail_content[3].select('td')[
                    5].text.strip()
                # 胎压检测
                tire_pressure_detection = detail_content[3].select('td')[
                    7].text.strip()
                # 车内中控锁
                interior_central_locking = detail_content[3].select('td')[
                    9].text.strip()
                # 儿童座椅接口
                child_seat_interface = detail_content[3].select('td')[
                    11].text.strip()
                # 无钥匙启动
                ...
                # 电动天窗
                electric_skylight = detail_content[4].select('td')[
                    1].text.strip()
                # 全景天窗
                panoramic_sunroof = detail_content[4].select('td')[
                    3].text.strip()
                # 电动吸合门
                electric_pull_in_door = detail_content[4].select('td')[
                    5].text.strip()
                # 感应后备箱
                induction_trunk = detail_content[4].select('td')[
                    7].text.strip()
                # 感应雨刷
                induction_wiper = detail_content[4].select('td')[
                    9].text.strip()
                # 后雨刷
                rear_wiper = detail_content[4].select('td')[11].text.strip()
                # 前/后电动车窗
                front_rear_power_windows = detail_content[4].select('td')[
                    13].text.strip()
                # 后视镜电动调节
                rearview_mirror_electric_adjustment = detail_content[4].select('td')[
                    15].text.strip()
                # 后视镜加热
                rearview_mirror_heating = detail_content[4].select('td')[
                    17].text.strip()

                # 多功能方向盘
                multi_function_steering_wheel = detail_content[5].select('td')[
                    1].text.strip()
                # 定速巡航
                cruise_control = detail_content[5].select('td')[3].text.strip()
                # 后排独立空调
                rear_independent_air_conditioner = detail_content[5].select('td')[
                    5].text.strip()
                # 空调控制方式
                air_conditioning_control_mode = detail_content[5].select('td')[
                    7].text.strip()
                # gps
                ...
                # 前/后排座椅加热
                front_rear_seat_heating = detail_content[5].select('td')[
                    17].text.strip()
